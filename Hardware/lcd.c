	...
	if(lcddev.id==0x9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963||lcddev.id==0X9488)//9341/6804/5310/5510/1963/9488,特殊处理
	{
		...
		if(lcddev.id!=0X1963)//1963不做坐标处理
		{
			if(regval&0X20)
			{
				...
				{
					Temperature=lcddev.width;
					lcddev.width=lcddev.height;
					lcddev.height=Temperature;
				}
			}else  
			{
				if(lcddev.width>lcddev.height)//交换X,Y
...
{ 										  
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_FSMC,ENABLE);	//使能FSMC时钟
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;				 // 推挽输出 高电平  tft复位 改为->PE2   PG11(*)
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOG, &GPIO_InitStructure);	
	//复位
	delay_ms(100);
	GPIO_ResetBits(GPIOG,GPIO_Pin_11);  //输出低
	delay_ms(100);
	GPIO_SetBits(GPIOG,GPIO_Pin_11);  //输出高
	delay_ms(100);
	
	
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;				 //PB0 推挽输出 背光  --->PB15 PG6(*)
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOG, &GPIO_InitStructure);
//	GPIO_SetBits(GPIOG,GPIO_Pin_6);  //输出高
	...
	//PORTE复用推挽输出  RS-PE2:FSMC_A23(*)
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;				 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOE, &GPIO_InitStructure);    	    	 											 

   	//	//PORTG12复用推挽输出 A0	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;	 //	//PORTD复用推挽输出  FSMC_NE4(*)
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOG, &GPIO_InitStructure); 
...
{  							  
  u8 Temperature,t1,t;
	...
	for(t=0;t<csize;t++)
	{   
		if(size==12)Temperature=asc2_1206[num][t]; 	 	//调用1206字体
		else if(size==16)Temperature=asc2_1608[num][t];	//调用1608字体
		else if(size==24)Temperature=asc2_2412[num][t];	//调用2412字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(Temperature&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
{         	
	...
	for(t=0;t<len;t++)
	{
		Temperature=(num/LCD_Pow(10,len-t-1))%10;
		if(enshow==0&&t<(len-1))
		{
			if(Temperature==0)
			{
	 	...
	 	LCD_ShowChar(x+(size/2)*t,y,Temperature+'0',size,0); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
{  
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
	 	...
	 	LCD_ShowChar(x+(size/2)*t,y,Temperature+'0',size,mode&0X01); 
	}
} 
//显示字符串
//x,y:起点坐标
...
//在指定位置显示一个汉字
//x,y:起始坐标
void LCD_ShowGB2312(u16 x,u16 y,u8 size,u8 *buff)
{  							  
  u8 t1,t;
	u16 y0=y;
